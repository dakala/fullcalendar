    ...
    if (view.name == 'month') {
      ...
      for (var i in fields) {
        ...
        $.each(['min', 'max'], function (_, type) {
          $cal.find('#edit-' + fields[i] + '-' + type + '-year')
            .attr('value', date_parts[type][0]);
          $cal.find('#edit-' + fields[i] + '-' + type + '-month')
            .attr('value', date_parts[type][1]);
          $cal.find('#edit-' + fields[i] + '-' + type + '-day')
            .attr('value', date_parts[type][2]);
        });
      }
    }
